...
**Resubmit your completed notebook back to GitHub by 9:00 PM.**
...
time limit has expired, upload only your modified `test.Rmd` to GitHub.

Request: Do not use the `scales` library as it may cause a conflict with
`read_csv`.
...
- `Tenure` is the customer's length of employment at his or her current
job;
...
- `Grade` is the bank's internal letter grade given to the loan, where
"A" is the top grade.
...
Treat the empty string, "NA", and "n/a" as missing values.
...
Remove all rows of `df` having at least one missing value.
...
Calculate half the median loan amount and save it in a variable called
`half_med`.
...
answer. Instead, use R functions to calculate it.]
...
plot to a file called `graphic.png`. [Note: Just do the basic plot. Do
not add titles; do not try to make the plot beautiful.]
...
the columns, and average FICO score in the table values.
...
`Origination` properly converted to the `Date` class.
